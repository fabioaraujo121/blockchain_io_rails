    ...
    module Api
        class Wallet
            def self.create(args = {})
                ...
                pwd = BlockchainIoRails::encode_password(BlockchainIoRails::generate_password)
            ...
            def self.outgoing(user, to, amount)
                ...
                res = Net::HTTP.post_form(uri, 'password' => BlockchainIoRails::decode_password(user.wallet_password), 'to' => to, 'amount' => satoshi)
                if res.code == 200
                    puts res.body
                    {
            ...
            def self.address_balance(user, address = nil)
                address ||= user.wallet
                uri = URI("http://localhost:3000/merchant/" + user.guid + "/address_balance?password=" + BlockchainIoRails::decode_password(user.wallet_password) + "&address=" + address)
                Net::HTTP.get(uri)
            end
        end
    end
